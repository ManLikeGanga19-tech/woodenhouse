...
export default function Hero({ images }: { images: string[] }) {
  ...
  return (
    <section className="relative w-full h-[85vh] overflow-hidden select-none">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-white/90 text-lg md:text-xl mt-4 max-w-2xl drop-shadow-md leading-relaxed">
          Sustainable, durable, and beautifully crafted wooden houses designed for African climates.
        </p>

        <a
          href="/contact"
          className="mt-8 inline-block text-white text-lg font-semibold px-10 py-3 rounded-lg shadow-lg transition-all duration-300 hover:scale-[1.03]"
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3">
        {images.map((_, index) => (
          <div
            key={index}
            className={`w-3 h-3 rounded-full transition-all duration-300 ${current === index ? "scale-110" : "bg-white/60"
              }`}
